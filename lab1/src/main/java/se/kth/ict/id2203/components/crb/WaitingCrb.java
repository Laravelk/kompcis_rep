...
public class WaitingCrb extends ComponentDefinition {
	...
	private int[] vector;
	private List<CrbMessage> pending;
	...
	private Handler<CrbBroadcast> bcastHandler = new Handler<CrbBroadcast>() {
		...
		public void handle(CrbBroadcast event) {
			...
			CrbMessage msg = new CrbMessage(self, event.getDeliverEvent(), newVector);
			trigger(new RbBroadcast(msg), rb);
		}
	};

	private Handler<CrbMessage> crbDeliver = new Handler<CrbMessage>() {
		...
		public void handle(CrbMessage event) {
			...
			Collections.sort(pending, new Comparator<CrbMessage>() {
				public int compare (CrbMessage obj0, CrbMessage obj1) {
			...
			Iterator<CrbMessage> pendingIter = pending.iterator();
			CrbMessage tmpMsg;
